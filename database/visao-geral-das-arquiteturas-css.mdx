...
Esse module foi proposto por Nicole Sullivan, em 2008. O seu objetivo era aplicar conceitos de POO já conhecidos, principalmente através das linguagens Java e C#.
...
Em adição, Sullivan sugere a separação entre o *container* e conteúdo para evitar ao máximo a dependência de localização do objeto na árvore de renderização do HTML - o famoso DOM, Document Object Model -. Lembre-se que é uma prática manter um elemento com a mesma aparência, não importa onde estiver.
...
- Evite usar seletor descendente (como no explicado no último exemplo).
- Apenas use *!important* em casos extremos.

## SMACSS

Agora vamos falar de SMACSS (lê-se ****smacks).**** Foi criado por Jonathan Snook. O autor tem um livro sobre e um [site](https://smacss.com/) de documentação.
...
Nessa parte são os estilos relacionados a um “estado”, como estar selecionado, estar ativo, ou que contenha algo. Aqui há uma estreita relação com Javascript para aplicar ou retirar esses estilos dinamicamente.  Comumente são usados prefixos como “is-” e “has-” na nomenclatura de classes.
...
Importante ressaltar que SMACSS não te diga regra alguma, é apenar um guia. 

Uma opção de estrutura de pastas pode ser a seguinte, como exemplo:
...
Uma variação ou extensão do bloco. Um link ativo é um modificador específico do bloco.
...
Estas são apenas 3 das estratégias de manter os códigos CSS organizados conforme aumenta sua complexidade. Agora que você viu uma geral sobre cada uma delas, escolha qual te agrada mais, explore a documentação e comece a pôr em prática. Bons estudos! 👋
