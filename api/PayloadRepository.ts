import { IArticle, IArticlePreview, IDbArticleRepository } from "types/article-protocols";
    ...
    IDbArticleRepository
{
    ...
    async getArticle(slug: string): Promise<IArticle> {
        const response = await fetch(this.url + '/articles?where[slug][equals]=' + slug).then(d => d.json());
        const { docs } = response;

        return serializeArticle(docs[0])
    }
    async listArticlesPreview(): Promise<IArticlePreview[]> {
        const response = await fetch(this.url + '/articles').then(d => d.json());
        const { docs } = response;
...
const buildMarkdownLines = (nestedObject: any[]): string => {
    return nestedObject.reduce(((total, line) => total + '\n' + line.children[0].text), '')
}
